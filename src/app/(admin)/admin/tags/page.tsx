...
import { Tag, Plus, Pencil, Trash2, X, AlertTriangle } from "lucide-react";
...
export default function TagsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/tags?id=${deleteId}`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto select-none">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Tag Kategori</h2>
          <p className="text-xs text-zinc-550 dark:text-zinc-400 mt-1">
            Kelola label tag klasifikasi metadata artikel berita Portal Hi Aceh
          </p>
        </div>
        <button
              ...
              {tags.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-sm text-zinc-500">
                    Belum ada label tag terdaftar.
                  </td>
                </tr>
              ) : (
                tags.map((tag) => (
                  <tr key={tag.id} className="text-xs hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors">
                    <td className="py-4 px-4 font-semibold text-zinc-800 dark:text-zinc-200">
                      #{tag.name}
                    </td>
                    <td className="py-4 px-4 text-zinc-500 font-mono">{tag.slug}</td>
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-1 rounded border border-zinc-200 dark:border-zinc-800 hover:bg-red-50 dark:hover:bg-red-950/20 text-zinc-500 dark:text-zinc-400 hover:text-red-650 dark:hover:text-red-400 transition-colors cursor-pointer"
                          title="Hapus Tag"
                        >
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs font-mono rounded-lg border border-zinc-200 dark:border-zinc-855 bg-zinc-105 dark:bg-zinc-950 px-3 py-2.5 text-zinc-400 dark:text-zinc-505 focus:outline-none select-none border-zinc-200 dark:border-zinc-800"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-zinc-100 dark:border-zinc-900">
                ...
                >
                  {actionLoading ? "Menyimpan..." : "Simpan Tag"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6 leading-relaxed">
              Apakah Anda yakin ingin menghapus label tag ini? Menghapus tag akan melepas labelnya dari semua artikel terkait secara otomatis.
            </p>
