...
export default function EditAdPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const locations = data
            .filter((ad: any) => ad.id !== id)
            .map((ad: any) => ad.location);
    ...
    try {
      ...
      if (res.ok) {
        setFormData({
          ...
          location: data.location || "HEADER_TOP",
        ...
        setError(data.error || "Gagal memuat detail iklan.");
      }
    } catch (e) {
      console.error(e);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Gagal menyimpan perubahan iklan.");
      } else {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-550 dark:text-zinc-400 mt-1">
              Perbarui pengaturan dan materi iklan Anda
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="w-full max-w-2xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-6 flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-150 dark:border-zinc-900">
              <button
                type="button"
                onClick={() => setIsMediaModalOpen(false)}
                className="px-4 py-2 text-xs font-semibold rounded-lg border border-zinc-200 dark:border-zinc-800 text-zinc-600 hover:bg-zinc-50 dark:text-zinc-300 transition-colors cursor-pointer"
              >
                Tutup
              </button>
            </div>
